...
TLS(Transport Layer Security)는 SSL(Secure Socket Layer)의 문제점을 고쳐서 보다 개선된 버전이라고 볼 수 있다. SSL은 `POODLE` 혹은 `DROWN` 등의 취약점이 발생해서 쓰진않고 **TLS** 가 표준이나 `openSSL` 과 같은 오픈소스 프로젝트를 보면 알 수 있듯이 **SSL** 라는 말이 계속 쓰이고 있는 상황이다.

즉, 오늘날의 HTTPS는 **모두 TLS로 동작**한다고 생각하면 된다. 그렇다면 이런 부분이 궁금해질 수 있다고 생각한다.

**TLS** 는 **HTTP** 랑만 사용하나요?? 이건 아니다. 대표적으로 FTPS(File Transport protocol over SSL)과 같은게 있다 (SFTP는 ssh를 사용하므로 엄밀히 따졌을때 TLS를 사용하지는 않는다.)

만약, FTPS와 SFTP의 차이가 궁금하다면 [ftp, ftps, sftp(ssh) 개념 정리](https://nhj12311.tistory.com/76)를 참고해보자. 

지금까지 HTTPS 스택에 대해서 알아봤다. 그렇다면 네트워크에서 3way-handshake 나 4way-handshake가 있듯이 HTTPS에도 서버 <-> 클라이언트간의 연결수립 및 해제 방법이 존재할 것이다.
...
사실 위 내용은 아주 간단하게 HTTPS Handshake를 요약해둔 것이다.
 `Server Hello` 단계에서도 내부적으로 좀 더 스텝들이 존재하나 이 부분이 궁금하다면 [TLS Security 5: Establishing a TLS Connection | Acunetix](https://www.acunetix.com/blog/articles/establishing-tls-ssl-connection-part-5/) 이 아티클을 참고하는 것을 추천한다.

HTTPS는 위와 같이 연결 수립과정에서는 비대칭키를 활용하여 세션 키를 만든 후에 연결 수립이 완료된 후에는 이 키를 대칭키 암호화 형식으로 주고 받는 식으로 보안성을 높였다.
...
매우 간단하다.
`brew install minica` 면 끝난다.

윈도우나 리눅스 환경은 [GitHub - jsha/minica](https://github.com/jsha/minica) 여기서 문서를 확인해보자.

2. minica로 `localhost` 인증서 발급 
`minica -domains www.localhost,localhist -ip-addresses 127.0.0.1` 와 같이 명령어를 입력해주자.

![result-command](./Screen%20Shot%202022-03-24%20at%2021.38.36.png)

그러면 위와 같이 `pem` 파일들이 생성된 것을 확인할 수 있다. 매우 간단하게 `CA` 인증서를 만든 것이다. 

물론, 이 `minica.pem` 으로도 `Apache` 나 `nginx` 같은데 올려서 `HTTPS` 적용을 할 수 있긴하다.

그러나, 이 `.pem` 파일을 스프링 부트 내장톰캣에 끼워서 띄워야하는데 톰캣은 `.jks(Java KeyStore)` 를 지원하기 때문에 변환 작업이 필요한 것이다.


+ nginx.conf 
![nginx.conf](./Screen%20Shot%202022-03-24%20at%2021.57.09.png)

실제로 생성된 인증서(pem)을 통해서 nginx에 등록하면 이러한 화면을 볼 수 있다.


![nginx-result](./Screen%20Shot%202022-03-24%20at%2021.58.10.png)
...
![result-command](./Screen%20Shot%202022-03-24%20at%2022.02.59.png)

이를 마치면 `.p12` 파일이 생성됨을 확인할 수 있다.
![result-command](./Screen%20Shot%202022-03-24%20at%2022.03.11.png)
...
![result-command](./Screen%20Shot%202022-03-24%20at%2022.08.34.png)

위의 처럼 `.jks` 가 생성됐으면 성공적으로 생성된 것이다.
...
스프링 부트 초기 설치는 생략하고 (의존성은 `boot-starter-web` 만 있어도 된다.)
바로 스프링 부트 설정을 보도록 하자.
...
server:
  ...
  ssl:
    ...
    key-store-password: "zxcv1234"  # .jks를 생성할 때 입력했던 keystore 비밀번호
    key-password: "zxcv1234" # .jks를 생성할 때 입력했던 key 비밀번호
    key-alias: "localhost" # .jks 별칭
``` 

이제 스프링 부트를 동작시켜보자.

![run-spring](./Screen%20Shot%202022-03-24%20at%2022.12.43.png)

`http://localhost:8080` 로 접근하면 아래와 같은 메시지가 나올 것이다. 

![result-localhost](./Screen%20Shot%202022-03-24%20at%2022.13.08.png)

다시 `https://localhost:8080` 로 접근을 하면 아래와 같이 나올 것이다.

![result-chrome](./Screen%20Shot%202022-03-24%20at%2022.21.16.png)

![result-ca](./Screen%20Shot%202022-03-24%20at%2022.22.29.png)
...
![keychain](./Screen%20Shot%202022-03-24%20at%2022.24.59.png)

스포트 라이트에 `keychain` 이라 검색하여 위와 같은 인증서를 볼 수 있는 설정으로 들어간 후에

![keychain-option](./Screen%20Shot%202022-03-24%20at%2022.24.53.png)

여기서 `import Items` 를 통해서 인증서를 추가하면 끝난다.

![clear](./Screen%20Shot%202022-03-24%20at%2022.26.36.png)
